...
export default function Inbound() {
  ...
  const handleDelete = async (id) => {
    if (!confirm('确定删除该入库记录？将同步扣减库存。')) return;
    try {
    ...
    <div>
      <div className="card">
        <div className="card-header">
          <h3>入库记录 ({total})</h3>
          <button className="btn btn-success" onClick={() => setModalOpen(true)}>+ 新增入库</button>
        </div>
        ...
        {loading ? <div className="empty"><p>加载中...</p></div> :
          records.length === 0 ? <div className="empty"><div className="icon">📥</div><p>{search ? '未找到匹配的入库记录' : '暂无入库记录'}</p></div> :
              ...
              <tbody>
                {records.map(r => (
                  <tr key={r.id}>
                    ...
                    <td style={{ fontWeight: 700, color: 'var(--success)', fontSize: 16 }}>+{r.quantity}</td>
                    <td>{r.batch_no || '-'}</td>
                    <td>{r.supplier || '-'}</td>
